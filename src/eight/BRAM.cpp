...
bool BRAM::isleafbn(BTreeNode* bn)
{
	if(bn->cpos[0]==-1)
		return true;
	else return false;
...
Bucketb BRAM::DeserialiseBucketb(block buffer) 
{
    ...
    for (int z = 0; z < Z; z++) 
    {
        ...
        BTreeNode* commo = convertBlockbToBTreeNode(block.data);
        block.id = commo->bid;
	free(commo);
        //delete commo; 
...
{
    BTreeNode* node = (BTreeNode*)malloc(sizeof(BTreeNode));//new BTreeNode();
...
DONE: cout <<"done INDEX setup"<<endl;
}
