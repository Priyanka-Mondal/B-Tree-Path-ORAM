...
fblock AES::EncryptBlock(bytes<Key> key, bytes<IV> iv, fblock plaintext,size_t clen_size,size_t plaintext_size) {
    fblock ciphertext(clen_size);
    EncryptBytes(key, iv, plaintext.data(), plaintext_size, ciphertext.data());
    return ciphertext;
}

fblock AES::DecryptBlock(bytes<Key> key, bytes<IV> iv, fblock ciphertext,size_t clen_size) {
...
fblock AES::Encrypt(bytes<Key> key, fblock plaintext,size_t clen_size,size_t plaintext_size) {
    fblock ciphertext;
...
fblock AES::Decrypt(bytes<Key> key, fblock ciphertext,size_t clen_size) {
    // Extract the IV
    bytes<IV> iv;
