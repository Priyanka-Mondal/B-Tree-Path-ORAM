...
ORAMf::ORAMf(int maxSize, bytes<Key> oram_key)
: key(oram_key), dis(0, (int) (pow(2, floor(log2(maxSize / Z)) + 1) - 1) / 2) {
    ...
    store = new RAMStoref(storeBlockCount, storeBlockSize);
...
int ORAMf::GetNodeOnPath(int leaf, int curDepth) {
    leaf += bucketCount / 2;
...
block ORAMf::SerialiseBucket(Bucket bucket) {
    block buffer;
...
Bucket ORAMf::DeserialiseBucket(block buffer) {
    assert(buffer.size() == Z * (blockSize));
...
Bucket ORAMf::ReadBucket(int index) {
    totalRead++;
...
void ORAMf::WriteBucket(int index, Bucket bucket) {
    totalWrite++;
...
void ORAMf::WriteData(Bid bid, Node* node) {
    cache[bid] = node;
}

// Fetches a block, allowing you to read and write in a block

void ORAMf::Access(Bid bid, Node*& node, int lastLeaf, int newLeaf) {
    FetchPath(lastLeaf);
    node = ReadData(bid);
    if (node != NULL) {
        node->pos = newLeaf;
...
void ORAMf::Access(Bid bid, Node*& node) {
    if (!batchWrite) {
        FetchPath(node->pos);
    }
    WriteData(bid, node);
...
Node* ORAMf::ReadNodeFromCache(Bid bid) {
    if (bid == 0) {
...
Node* ORAMf::ReadNode(Bid bid, int lastLeaf, int newLeaf) {
    if (bid == 0) {
        return NULL;
    }
...
int ORAMf::WriteNode(Bid bid, Node* node) {
    if (bid == 0) {
        throw runtime_error("Node id is not set");
    }
    if (cache.count(bid) == 0) {
...
Node* ORAMf::convertBlockToNode(block b) {
    Node* node = new Node();
...
block ORAMf::convertNodeToBlock(Node* node) {
...
void ORAMf::start(bool isBatchWrite) {
...
void ORAMf::setupInsert(vector<Node*> nodes) {
