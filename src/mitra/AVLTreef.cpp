...
AVLTreef::AVLTreef(int maxSize, bytes<Key> key) {
...
Node* AVLTreef::newNode(Bid key, string value) {
    Node* node = new Node();
    node->key = key;
...
Node* AVLTreef::rightRotate(Node* y) {
...
Node* AVLTreef::leftRotate(Node* x) {
...
int AVLTreef::getBalance(Node* N) {
    if (N == NULL)
        return 0;
 ...
 */
Node* AVLTreef::search(Node* head, Bid key) {
 ...
 * a recursive search function which traverse binary tree to find the target node
 */
void AVLTreef::batchSearch(Node* head, vector<Bid> keys, vector<Node*>* results) {
    if (head == NULL || head->key == 0) {
        return;
    }
    head = oram->ReadNode(head->key, head->pos, head->pos);
...
void AVLTreef::printTree(Node* root, int indent) {
